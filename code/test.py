...
def test(model_path_effi7, output_dir, test_loader, addNDVI):
    in_channels = 4
    if(addNDVI):
        # print(1)
        in_channels += 1
    ...
    for image, image_stretch, image_path, ndvi in test_loader:
        ...
        output = (output3 + output4 + output6 + output8) / 4.0
...
if __name__ == "__main__":
    ...
    # model_path_effi7 = ".\model/unetplusplus_dpn131_upsample_SoftCE_dice.pth"
    model_path_effi7 = ".\model/unetplusplus_dpn500_Lovasz_upsample_SoftCE_Lovasz.pth"
    # model_path_resnest = "model/unetplusplus_resnest_upsample_SoftCE_dice.pth"
    output_dir = '.\prediction_result/'  
    test_image_paths = glob.glob(r'G:\test\rs_Unet\Pytorch-Semantic-segmentation-of-remote-sensing\data\round1_test/*.tif')
    # r'G:\test\rs_Unet\Pytorch-Semantic-segmentation-of-remote-sensing\data\round1_train\*.png'
    # print(test_image_paths)
    addNDVI = False
    batch_size = 16
    num_workers = 8
    test_loader = get_dataloader(test_image_paths, None, "test", addNDVI, batch_size, False, 4)
    test(model_path_effi7, output_dir, test_loader, addNDVI)
    # test_1(model_path, output_dir, test_loader, addNDVI)
    print((time.time()-start_time)/60**1)

# t1 = r'G:/test/rs_Unet/Pytorch-Semantic-segmentation-of-remote-sensing/data/round1_test'