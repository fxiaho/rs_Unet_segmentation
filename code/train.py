import time
now1 = time.localtime()
print("开始时间：" + time.strftime("%Y-%m-%d %H:%M:%S", now1))
...
now2 = time.localtime()
print("数据预处理时间：" + time.strftime("%Y-%m-%d %H:%M:%S", now2))
...
def train(EPOCHES, BATCH_SIZE, train_image_paths, train_label_paths, 
    ...
    if(loss == "SoftCE_dice"):
        ...
        # 定义组合损失函数
        def custom_loss1(y_pred, y_true):
           
            # 这里可以根据需要调整权重
            total_loss = 0.5 * DiceLoss_fn(y_pred, y_true) + 0.5 * SoftCrossEntropy_fn(y_pred, y_true)
            return total_loss
    

        # 使用组合损失函数
        loss_fn = custom_loss1
    else:
        # 损失函数采用SoftCrossEntropyLoss+LovaszLoss
        # LovaszLoss是对基于子模块损失凸Lovasz扩展的mIoU损失的直接优化
        LovaszLoss_fn = LovaszLoss(mode='multiclass').to(DEVICE)
        # 软交叉熵,即使用了标签平滑的交叉熵,会增加泛化性
        SoftCrossEntropy_fn=SoftCrossEntropyLoss(smooth_factor=0.1).to(DEVICE)
        ...
        def custom_loss2(y_pred, y_true):
            ...
            # 这里可以根据需要调整权重
            total_loss = 0.5 * LovaszLoss_fn(y_pred, y_true) + 0.5 * SoftCrossEntropy_fn(y_pred, y_true)
            return total_loss
...
#   进行可视化参数设置
    ...
    for epoch in range(1, EPOCHES+1):
        ...
        model.train()
        model.to(DEVICE);
        prt1="\rEpoch: {:d} ".format(epoch)  #显示进度
        

        for batch_index, (image, target) in enumerate(train_loader):
            image, target = image.to(DEVICE), target.to(DEVICE)

            prt2="batch: {:d} ". format(batch_index+1) #显示进度
            time.sleep(0.1)
...
#                scaler.update()
            ...
            ## 计算准确率（这只是一个示例，请根据实际情况修改）
            # accuracy = calculate_accuracy(output, target)
            # accuracies.append(accuracy)  # 存储每个batch的准确率
            # 画图
            count += 1
        ...
        if best_miou < np.stack(val_iou).mean(0).mean():
            ...
            if (epoch - best_miou_epoch) >= early_stop:
                break
    # 打印当前epoch的信息 统计进度
        print("Epoch [{}/{}], Train Loss: {:.4f}, Time: {:.2f} min".format(epoch, EPOCHES, train_loss, (time.time()-start_time)/60))    
...
if __name__ == '__main__':
    ...
    # BATCH_SIZE = 4  #原始
    BATCH_SIZE = 8  #改
    image_paths = glob.glob(r'data\round1_train\*.tif')
    label_paths = glob.glob(r'data\round1_train\*.png')
    ...
    model_path = "model/unetplusplus_dpn500_Lovasz"
    if(upsample):
    ...
    train_loss_epochs, val_mIoU_epochs, lr_epochs = train(EPOCHES, 
                                                          ...
                                                          early_stop)
    now3 = time.localtime()
    print("结束时间："  + time.strftime("%Y-%m-%d %H:%M:%S", now3))
    consumed_time = time.mktime(now3) - time.mktime(now1)
    print("消耗时间："  + time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(consumed_time)))
    ...
    if(True):    
        ...
        plt.show() 
        now4 = time.localtime()
        print("结束时间：" + time.strftime("%Y-%m-%d %H:%M:%S", now4))
